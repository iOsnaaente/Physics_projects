...
if __name__ == '__main__': 
    ...
    loc  = Location( latitude= -29.16530765942215, longitude= -54.89831672609559, temperature= 298.5 )   

    now = datetime.now()
    time = Time( 2021, now.month, now.day, now.hour, now.minute, now.second )
    
    sun = Solar( loc, time, Position )
    sun.compute( time )
    ...
    for i in range( 3600//5 ):
        ...
        if minuto >=60:
            ...
            if hora >= 24:
                hora -= 24
                dia += 1  
        time = Time( 2021, 6, dia, hora, minuto, 0 ) 
        sun.compute( time )
        x.append( dia*24*60 + hora*60 + minuto )
        azimute.append( sun.get_azimute() )
        altitude.append( sun.get_altitude() )
    
    plt.plot( x, azimute, 'r' )
    plt.plot( x, altitude, 'b' )
    plt.show() 
